...
def search_combined_queries():
    ...
    typer.echo(
        f"Searching for:\n must match category: Vegan;\
         \n must not match ingredients: peanuts\
          \n filter sodium levels on the range: 100-140mg\n"
    )
    query_body = {
        "query": {
            "bool": {
                "must": {"match": {"categories": "vegan"}},
                "must_not": {"match": {"ingredients": "peanuts"}},
                "filter": [{"range": {"sodium": {"gte": 100, "lte": 140}}}],
            }
        }
    }
    resp = client.search(index=INDEX_NAME, body=query_body)
